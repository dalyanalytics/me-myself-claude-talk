...
# Create data for overlapping circles representing professional joy components
circles <- data.frame(
  x = c(2, 4, 3, 3),
  y = c(2.5, 2.5, 4, 1),
  r = c(1.5, 1.5, 1.3, 1.3),
  label = c("Client\nWork", "Technical\nGrowth", "Community", "Life\nBalance"),
  color = c("#6A94C9", "#AD92B1", "#D68A93", "#F9B397")
)

ggplot() +
  geom_circle(data = circles, aes(x0 = x, y0 = y, r = r, fill = color),
              alpha = 0.3, color = NA) +
  geom_text(data = circles, aes(x = x, y = y, label = label),
            size = 5, fontface = "bold", color = "#404041") +
  annotate("text", x = 3, y = 2.5, label = "Professional\nJoy",
           size = 6, fontface = "bold", color = "#404041") +
  scale_fill_identity() +
  coord_fixed() +
  theme_void() +

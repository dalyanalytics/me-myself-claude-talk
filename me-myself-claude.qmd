...
- **"I'm Learning"** → Honest acknowledgment of exploration
:::
...
## The {shinyfa} Package ![](images/shinyfa.png){.absolute top=50 right=50 width="150"}

::: {.incremental}
- **The Problem:** Hired to onboard to a large, complex Shiny app for a conservation nonprofit
- **The Reality:** Took me *months* to get comfortable navigating their codebase
- **The Insight:** "There has to be a better way to understand file relationships and reactivity patterns"
- **The Solution:** Started with a one-off script to analyze their app structure
- **The Evolution:** Refined those functions → generalized the approach → built {shinyfa}
- **The Impact:** A tool that would have saved me weeks of onboarding time
:::
...
The real-world story: I was hired by a conservation nonprofit to work on their large Shiny application. It took me MONTHS to get comfortable in their repo - understanding which files did what, how the reactive dependencies flowed, where inputs and outputs were defined. It was a significant onboarding challenge.

About halfway through the project, I had this realization: "I'm a Shiny developer. There has to be a better way to systematically analyze app structure." So I wrote a one-off script to help me catalog the render functions, reactive functions, inputs, and file relationships in their app.

That script was so useful that I kept refining it. Then I thought - "Other developers face this same problem." With Claude's help, I generalized those functions, added proper documentation, built tests, and packaged it up as {shinyfa}.

The entire process from "useful script" to "published package" took weeks, not months. Without AI assistance, this would have stayed as a one-off script on my hard drive, or been a "someday" project I never got to.

This is what mental bandwidth buys you: the ability to take a client problem, generalize it, and contribute a tool back to the community - all without sacrificing your business.
:::
...
::: {.incremental}
- **The Discovery:** Started birding in 2025 - found the AviList Global Avian Checklist (an Excel sheet!)
- **The Realization:** "R birders need this taxonomy data... but it's in Excel"
- **The Solution:** Wrapped the published Excel sheet into an R package with Cornell Lab species codes
- **The Value Add:** Created starter code showing how to integrate with eBird data via {rebird}
- **The Impact:** 11,131 species, unified taxonomy, ready for R pipelines
:::
...
::: {.notes}
Total honesty: this wasn't some groundbreaking technical achievement. I took a published Excel sheet (the AviList Global Avian Checklist) and wrapped it into an R package. The Excel conversion itself was straightforward - not a heavy lift at all.

I started birding this year and discovered this unified global bird taxonomy. For R users who want to work with bird data - especially integrating with eBird through the {rebird} package - having this taxonomy accessible in R makes workflows much cleaner.

The real work - and where AI made the difference - was creating the starter code and documentation. The getting-started vignette shows practical workflows: exploring the 11,131 species, finding specific groups (like all 194 thrush species), comparing naming differences between taxonomies, and connecting to real-time eBird observations.

With AI assistance, I could focus on "what examples would actually help birders?" rather than getting bogged down in the tedium of writing documentation. Claude helped me structure the vignettes, write clear examples, and make the package immediately useful to someone who installs it. The entire process - from idea to published on CRAN with comprehensive documentation - took less than a week.

This is about lowering the barrier to contribution. You don't need to build something technically complex to add value to a community. Sometimes it's just "take this useful thing in the wrong format and make it accessible to your community" - and with AI, doing that well (with good docs!) becomes feasible in days, not months.
...
- {ttbbeer} (July 2016) → {shinyLP} (September 2016) → ... **8 years 9 months** ... → {avilistr} (June 2025)
:::
...
::: {.notes}
Let's talk about what this really means. I'm not becoming dependent on Claude - I'm accelerating my growth as an R developer.

The timeline is striking: {ttbbeer} my first package was released July 2016, {shinyLP} my second package September 2016. Then... nothing on CRAN until {avilistr} in June 2025. That's 8 years 9 months (September 16, 2016 – June 17, 2025)

Why the gap? Life got busy. Client work took priority. Building packages felt like a luxury I couldn't afford. The friction of "I have to write all the docs, build the vignettes, structure everything properly" was enough to keep good ideas as one-off scripts on my hard drive.

But here's what changed with AI: the friction disappeared. In 2025 alone, I've released two packages - {shinyfa} and {avilistr} - both with comprehensive documentation, working examples, and proper structure. Not because I got lazy and let AI do everything, but because AI handled the tedious parts while I focused on the creative, strategic parts.

Every package I build makes me a better R developer. I'm learning package architecture, documentation patterns, API design, community engagement. AI isn't making me dependent - it's removing the barriers that prevented me from practicing these skills in the first place.

The 8 year 9 month gap wasn't because I didn't have ideas. It was because the activation energy was too high. AI lowered that barrier, and suddenly I'm shipping packages, contributing to communities, and actively improving my R development skills through repeated practice.

This is the creative multiplier: when you remove the tedious friction, you don't get lazier - you get more prolific, and with each contribution, more skilled.
:::

...
<div style="font-size: 2.5em; color: #F9B397; font-weight: bold;">→</div>
...
for (file in file_path_df) {
  ...
  shiny_analysis$file_name <-
    basename(file)

  file_analysis <- bind_rows(
    file_analysis,
    shiny_analysis
  )
}
...
Catalog of render functions, reactive functions, inputs, and file relationships.
:::
:::

::: {.notes}
This shows the core workflow for using {shinyfa}. You point it at your Shiny app directory, and it systematically analyzes each R file.

**Left side - The Code:** The `analyze_shiny_reactivity()` function parses each file and extracts information about reactivity patterns. The code is straightforward R - iterate through files, analyze each one, bind results.

**Right side - The Output:** The result is a dataframe that gives you a complete map of your app's structure. Each row represents a reactive element: inputs, outputs, reactive expressions, and render functions. You can see which file they're in, what type they are, their name, and how they connect.
...
This structured view of your app's reactivity is what makes {shinyfa} useful for onboarding, documentation, and understanding complex Shiny applications. The value is enormous when you're trying to understand the architecture of a large Shiny app.
:::
...
**As a solo consultant, I'm not optimizing for maximum revenue—I'm optimizing for professional joy.**
...
::: {.fragment}
**AI assistance doesn't just free up time—it lets me be selective about where I invest it.**
:::
...
**Professional Joy as the North Star:** I'm optimizing for professional joy, which for me means working with clients whose missions I care about. Nonprofits, social impact organizations, NGOs—these are organizations doing important work in the world, often with limited resources. I find deep satisfaction in helping them leverage data and technology to amplify their impact.

**High Personal Interaction:** I'm not interested in being a code factory churning out anonymous projects. I want deep relationships with my clients—understanding their challenges, being a trusted partner, and seeing the long-term impact of the work we do together.
...
**Life Balance:** I'm a solo consultant, not a startup founder trying to build a unicorn. I have a life outside of work that matters—family, hobbies, health. Sustainable consulting means not burning out.
...
::: {.fragment}
This isn't about replacing your skills—it's about **amplifying your capacity** to take on bigger, more interesting problems.
:::
...
**The Challenge:** GitHub Actions YAML syntax is finicky. Scheduled workflows, manual testing pipelines, repository dispatch events for cross-repo triggers, configuration-driven scheduling—this is DevOps territory.
...
- "Add error handling" → "Add tryCatch with informative error messages for API failures"
- "Make it faster" → "Use data.table instead of dplyr for this 1M+ row operation"
...
- Stay in your lane—use AI to extend reach, not replace your expertise
:::

::: {.notes}
These are hard-won lessons from daily use of Claude Code in client work.

**Be Specific:** Vague instructions get vague results. The more specific you are about what you want, the better Claude's output will be. Think of it like managing a junior developer—clear requirements lead to better code.
...
- I'm not googling package documentation—I'm designing system architecture
...
**Use Claude Code for:**
- **Boilerplate:** Setting up Shiny modules, package structure, GitHub Actions templates—these follow known patterns. Let Claude handle it.
...
**Code Yourself for:**
- **Business logic:** The core algorithms that make your solution valuable—these need human understanding.
...
- **Security-sensitive code:** Authentication, authorization, data encryption—review these extremely carefully or write them yourself.
...
This framework fits perfectly in Part 3 because it's fundamentally about learning—understanding what tasks help you build R intuition (code yourself) versus what tasks are better delegated (use Claude). When you encounter Claude's mistakes (next slide), this framework helps you decide what to fix yourself versus when to redirect Claude.
:::
...
**Claude makes mistakes. This is actually valuable—here are real examples from building this presentation:**
...
- Claude used `avilist_global` dataset (doesn't exist—it's `avilist_2025`)
...
::: {.fragment}
**These mistakes made me a better R developer—I had to understand the packages deeply to fix them.**
:::
...
**The key insight:** Each of these mistakes forced me to develop deeper understanding. I couldn't just accept Claude's output—I had to understand the packages, data structures, and available functions well enough to correct the errors. This is exactly the kind of learning that builds intuition and makes you a better developer. You're not becoming dependent on AI; you're accelerating your learning by encountering (and fixing) more problems faster than traditional development would expose you to.
:::
